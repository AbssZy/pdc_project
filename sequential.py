...
#===================================================Euclidian distance=====================
def eucl_distance(point_one, point_two): #define function tomeasure Euclidian distance between point_one and point_two
if(len(point_one) != len(point_two)): #check for an error
(len(1st point)is not equal len(2nd point))
...
#print('div',np.divide((np.sum([data[i] for i in indices],axis=0)).astype(np.float),totcounter[k]))centroid[k-1]=np.divide((np.sum([data[i] for i in indices],axis=0)).astype(np.float),Q_clusts[k])
...
# fig = plt.figure()
# scatter = fig.scatter([x],[y],c=[Cluster],s=50)
